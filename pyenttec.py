...
def available_ports():
    ...
    if platform not in _port_basenames.keys():
...
class EnttecProParams(object):
    ...
    def to_packet(self):
        ...
        return b''.join(bytes([val]) for val in packet)
...
class DMXConnection(object):
    ...
    def _update_params(self):
        ...
        packet_start = [_START_VAL,
                        ...
                        ( (univ_size + 1) >> 8) & 0xFF,
                        0]
        self._packet_start = b''.join(bytes([v]) for v in packet_start)
    ...
    def render(self):
        ...
        dmx_payload = (bytes([v]) for v in self.dmx_frame)

        self.com.write(self._packet_start + b''.join(dmx_payload) + _PACKET_END)
